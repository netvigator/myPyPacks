...
#
# Copyright 2008-2012 Rick Graves
#
#
...
class DummyMeter( object ):
    ...
    def __init__(self, fo=stderr, use_hours=0, use_days=0):
        ...
        pass
        #
    def start(self, iSize=None, sShowText=None, sLineB4 = None ):
        #
        pass
        #
...
class TextMeter(BaseMeter):
    ...
    def _do_update(self, iDone, fNow=None):
        ...
        #self.iSize = None
        sShowText = self.sShowText
        if self.iSize is None:
            out = '\r%-60.60s    %8s %s ' % \
                  (sShowText, fread, fetime)
        else:
            ...
            out = self.sOutFormat % \
                  ( sShowText, frac*100, bar, fread, frtime )
    ...
    def _do_end(self, iDone, fNow=None):
        ...
        if self.iSize is None:
            out = '\r%-60.60s    %8s %s ' % \
                  (sShowText, iSizeFinal, total_time)
        else:
            bar = '='*25
            out = self.sOutFormat  % \
                  ( sShowText, 100, bar, iSizeFinal, total_time )
        self.fo.write(out + '\n' )
        self.fo.flush()
...
if __name__ == "__main__":
    ...
    iLen            = 100
    #
    sShowText       = "%s %s" % ( ReadableNo( iLen ), 'fast ticks' )
    #
    oProgressMeter.start( iLen , sShowText, 'passing the time ...' )
    #
    ...
    iLen            = 18005
    #
    sShowText       = "%s %s" % ( ReadableNo( iLen ), 'slow ticks' )
    #
    oProgressMeter.start( iLen , sShowText, 'passing the time ...' )
    #
    for iThis in iRange( iLen - 1 ):
