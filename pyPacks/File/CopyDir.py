...
def CopyAllFiles( sDirFrom, sDirTo, uWantExtensions = [] ):
    ...
    for sFile in lFiles:
        ...
        if isdir( sFileFrom ):
            ...
            try:
                #
                if iVerbose > 7: print3( 'copying', sFileFrom, 'to', sFileTo )
                #
                iCopiedFile = CopyOneFile(
                                sFileFrom, sFileTo, dWantExtensions = dWantExtensions )
                #
                ...
                #
                sType, sValue, oTraceBack = exc_info()
                print3( 'Error copying', sFileFrom, 'to', sFileTo, '-- skipped' )
                print3( sType, sValue )
            #
...
def main( sDirFrom = None, sDirTo = None, lWantExtensions = None ):
    #
    import time
    #
