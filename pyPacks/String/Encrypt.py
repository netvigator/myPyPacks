...
XOREncrypt( sTest )  = '3c0d094c1e1d0c0f074f0a17031b01480303144f0210011c0a0c45031a0a1a4518040a48090d16164801030b41'
...
def DescendChars( sOrig,
    ...
    if bStepIncrement:
        #
        iIncrement  = ( -1, 1 )[ not bBackStep ] # returns -1 if bBackStep, 1 if not
        #
    ...
    def getNewChar( sChar, iThis ):
        #
        return chr( ( 255 + iOffset + getIncrement( iThis ) - ord( sChar ) ) % 256 )
    #
    ...
    if bAlternate:
        ...
        def getFlip( s, i ):
            ...
            if i % 2:
                ...
                return s.swapcase()
        #
        sFlipped    = ''.join( iMap( getFlip, sText, iRange( len( sText ) ) ) )
        #
    else:
        #
...
def Encrypt( sEncryptThis, bUnEncrypt = False ):
    ...
    if bUnEncrypt:
        #
        sConvertChars   = DescendChars( FlipCase( sEncryptThis, 1 ), 0, 1 )
        #
    else: # Encrypt
        #
        # sConvertChars   = sEncryptThis
        sConvertChars   = ShuffleAndCut( getTextReversed( ShuffleAndCut( sEncryptThis ) ) )
        #
    #
    oStats              = AscStats( sConvertChars )
    #
    iUseOffset          = ( oStats.iLength % 4 ) - ( oStats.iDifference % 8 )
    #
    sConvertChars       = DescendChars( sConvertChars, iUseOffset )
    #
    if bUnEncrypt:
        #
        # pass
        sConvertChars   = ShuffleAndCut( getTextReversed( ShuffleAndCut( sConvertChars, 1 ) ), 1 )
        #
    else: # Encrypt
        #
        sConvertChars   = FlipCase( DescendChars( sConvertChars, 0, 1 ), 1 )
        #
    #
    #
    #
    return sConvertChars
...
def XOREncrypt( sText, sPassword = 'hello' ):
    ...
    #
    lText       = [ '%02x' % ( iText ^ iPassword, ) for iText, iPassword in lPairs ]
    #
    return ''.join( lText )
...
def XORdecript( sText, sPassword = 'hello' ):
    ...
    lChrs       = [ chr( iChr ^ iPass )
                    for iChr, iPass in iZip( lChrNumbs, tPassword * iPasswords ) ]
    #
    return ''.join( lChrs )
...
def EncryptLite( sEncryptThis, bUnEncrypt = 0 ):
    #
    from Collect.Cards  import ShuffleAndCut
    from String.Replace import getTextReversed
    #
    if bUnEncrypt:
        #
        sConvertChars   = changePunct(
                            FlipCase(
                                getTextReversed(
                                    getRot13(
                                        ShuffleAndCut( sEncryptThis, 1 ) ) ), 1 ) )
        # sConvertChars   = getRot13( sEncryptThis )
        #
    else: # Encrypt
        #
        # ShuffleAndCut(
        #
        sConvertChars   = ShuffleAndCut(
                            getRot13(
                                getTextReversed(
                                    FlipCase(
                                        changePunct( sEncryptThis ), 1 ) ) ) )
        #
    #
    return sConvertChars
...
def DecryptLite( sThis ):
    #
    return EncryptLite( sThis, bUnEncrypt = True )
...
if __name__ == "__main__":
    ...
    if      FlipCase( lowercase ) != uppercase or \
            ...
            FlipCase( lowercase, bAlternate = True ) != 'AbCdEfGhIjKlMnOpQrStUvWxYz' or \
            FlipCase( uppercase, bAlternate = True ) != 'aBcDeFgHiJkLmNoPqRsTuVwXyZ':
        #
    ...
    if      Encrypt( Encrypt( sTest ), bUnEncrypt = True ) != sTest or \
                     Encrypt( sTest ) == sTest:
        #
        lProblems.append( 'Encrypt()' )
        #
    if      EncryptLite( EncryptLite( sTest ), bUnEncrypt = True ) != sTest or \
    ...
    if Decrypt( Encrypt( sTest ) ) != sTest:
        #
        lProblems.append( 'Decrypt()' )
        #
    if DecryptLite( EncryptLite( sTest ) ) != sTest:
        #
        lProblems.append( 'DecryptLite()' )
        #
