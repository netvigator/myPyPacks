...
def ZipThis( sContent ):
    #
    from gzip           import GzipFile
    from Utils.Both2n3  import BytesIO, getBytes
    #
    oWriteable  = BytesIO()
    #
    oCompress   = GzipFile( fileobj = oWriteable, mode = 'wb' )
    #
    if PYTHON3 and isinstance( sContent, str ):
        #
        sContent = sContent.encode()
        #
    #
    oCompress.write( getBytes( sContent ) )
    #
    oCompress.close()
    #
    oWriteable.seek(0)
