...
iSecsPerDay     = 24 * 3600
tMagnitudes     = ( 1e-3, 1,  60,     3600, iSecsPerDay, iSecsPerDay * 24  )
tMultiplyBy     = ( 1e6,  1e3, 1, 1.0 / 60,  1.0 / 3600, 1.0 / iSecsPerDay )
tSayIncrement   = ( 'microseconds',
                    'milliseconds',
                    'seconds',
                    'minutes',
                    'hours',
...
def getDurationUnits( nSayTime ):
    ...
    #
    iWhich = bisect_right( tMagnitudes, nSayTime )
    #
    return tMultiplyBy[ iWhich ], tSayIncrement[ iWhich ]
...
if __name__ == "__main__":
    ...
    def getHalf( n ):
        if isInteger( n ) and ( n > 9 or isEven( n ) ):
            ...
            return n/2.0
    #
    tLessMags   = tMap( getHalf, tMagnitudes )
    #
    ...
    #
    tDurations = tMap( getBeforeAndNow, tMagnitudes )
    #
